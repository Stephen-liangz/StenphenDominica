...
@interface PresentVCTransition()

@property (nonatomic, assign) ZLPresntVCTransitionType type;

@end

@implementation PresentVCTransition
...
- (NSTimeInterval)transitionDuration:(id<UIViewControllerContextTransitioning>)transitionContext{
    return _type == ZLPresntVCTransitionTypePresent ? 0.5 : 0.25;
}

- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext{
    //为了将两种动画的逻辑分开，变得更加清晰，我们分开书写逻辑，
    switch (_type) {
        case ZLPresntVCTransitionTypePresent:
            [self presentAnimation:transitionContext];
            break;
            
        case ZLPresntVCTransitionTypeDismiss:
